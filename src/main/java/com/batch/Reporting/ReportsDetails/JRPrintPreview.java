...
public class JRPrintPreview extends VBox {
    ...
    protected final DecimalFormat zoomDecimalFormat = new DecimalFormat("#.##");

    protected final JasperPrint jasperPrint;
    protected final double screenResolution;
    protected final JasperReportsContext jasperReportsContext;
    protected ResourceBundle resourceBundle;

    protected final ToggleButton btnActualSize = new ToggleButton();
    protected final ToggleButton btnFitPage = new ToggleButton();
    protected final ToggleButton btnFitWidth = new ToggleButton();
    protected final Button btnFirst = new Button();
    protected final Button btnLast = new Button();
    protected final Button btnNext = new Button();
    protected final Button btnPrevious = new Button();
    protected final Button btnPrint = new Button();
    protected final Button btnZoomIn = new Button();
    protected final Button btnZoomOut = new Button();
    protected final Button hand = new Button();
    protected final Button arrow = new Button();
    protected final ComboBox<String> cmbZoom = new ComboBox<>();
    protected final Label lblStatus = new Label();
    protected final VBox vBoxPage = new VBox();
    protected final HBox statusBar = new HBox();
    protected final ScrollPane scrollPane = new ScrollPane();
    protected final ToolBar toolBar = new ToolBar();
    protected final TextField txtGoTo = new TextField();

    protected final DropShadow dropShadow = new DropShadow();
    protected int pageIndex;

    protected final BooleanProperty handControl = new SimpleBooleanProperty();
    ...
    public JRPrintPreview(JasperReportsContext jasperReportsContext, JasperPrint jasperPrint, ResourceBundle resBundle) {
        ...
        setWidth(width > 1000 ? width : 1000);
    }
    ...
    private void initComponents() {
        ...
        hand.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("/net/sf/jasperreports/view/images/last.GIF")))); // NOI18N
        hand.setTooltip(new Tooltip(resourceBundle.getString("last.page")));
        hand.setPrefSize(23, 23);
        hand.setOnAction(e -> onHandButton());
        hand.setFocusTraversable(false);

        arrow.setGraphic(new ImageView(new Image(getClass().getResourceAsStream("/net/sf/jasperreports/view/images/last.GIF")))); // NOI18N
        arrow.setTooltip(new Tooltip(resourceBundle.getString("last.page")));
        arrow.setPrefSize(23, 23);
        arrow.setOnAction(e -> onArrowButton());
        ...
        toolBar.getItems().addAll(btnActualSize, btnFitPage, btnFitWidth, new Separator(), hand, arrow, new Separator(), btnZoomIn, btnZoomOut, cmbZoom, new Separator(),
        ...
        vBoxPage.prefWidthProperty().bind(scrollPane.widthProperty());
    ...
    protected void refresh() {
        try {
            ...
            txtGoTo.setText(String.valueOf(pageIndex));

        } catch (JRException ex) {
            Logger.getLogger(JRPrintPreview.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void onBtnNextPressed() {
        pageIndex++;
        if (pageIndex > jasperPrint.getPages().size() - 1) {
    ...
    private void onHandButton() {

        vBoxPage.setCursor(Cursor.HAND);
        handControl.setValue(true);
    }

    private void onArrowButton() {
        vBoxPage.setCursor(Cursor.DEFAULT);
        handControl.setValue(false);
    }
    ...
    private void onFitPagePressed() {
        ...
        cmbZoom.getSelectionModel().clearSelection(); // Has to be before setRealZoomRatio() call because clearSelection() will clear cmbZoom text box
        setRealZoomRatio(Math.min(heightRatio, widthRatio));
    }
        ...
        Task<Boolean> task = new Task<>() {
            ...
            protected Boolean call() {
                try {
                    ...
                    JasperPrintManager printer = JasperPrintManager.getInstance(jasperReportsContext);

                    return printer.print(jasperPrint, false);
                } catch (JRException ex) {
                    ex.printStackTrace();
                } finally {
