    ...
    public void update() {
        try {
            if (this.model.getIsShown().getValue()) {
                ...
                Map<Long, Material> dataMap = (Map)dataBaseList.stream().collect(Collectors.toMap(Material::getId, Function.identity()));
        ...
        return new Task<Boolean>() {
            protected Boolean call() throws Exception {
                ...
                dataList.forEach((tableRecorde) -> {
                    Material databaseRecord = (Material)dataMap.get(tableRecorde.getId());
